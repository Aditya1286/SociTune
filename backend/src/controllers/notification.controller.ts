...
class NotificationController {
    public async getNotifications(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (notifications.length === 0) {
                ...
                const getUser = (index: number, defaultName: string, defaultAvatar: string, defaultClerkId: string) => {
                ...
                const getSong = (index: number, defaultTitle: string, defaultArtist: string, defaultArtwork: string) => {
    ...
    public async markRead(req: Request, res: Response, next: NextFunction) {
        try {
            const { id } = req.params;
            const userId = (req as any).auth.userId;
            await socificationService.markRead(userId, id as string);
    ...
    public async deleteNotification(req: Request, res: Response, next: NextFunction) {
        try {
            const { id } = req.params;
            const userId = (req as any).auth.userId;
            await socificationService.deleteNotification(userId, id as string);

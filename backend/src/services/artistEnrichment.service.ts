...
class ArtistEnrichmentService {
    public async enrichArtist(artistName: string): Promise<any> {
        // 1. Fetch official Spotify metadata first
        let spotifyData: any = null;
        try {
            const spotifyService = Singleton.instance<SpotifyService>(SpotifyService);
...
3. The response must be strictly valid JSON. Do not include comments.`;
        ...
        try {
            ...
            if (spotifyData?.imageUrl) artistDoc.imageUrl = spotifyData.imageUrl;
            if (spotifyData?.followers) artistDoc.followers = spotifyData.followers;
            if (spotifyData?.genres && spotifyData.genres.length > 0) artistDoc.genres = spotifyData.genres;
            if (spotifyData?.spotifyUrl) artistDoc.spotify = spotifyData.spotifyUrl;
            ...
            if (data.bio) artistDoc.bio = data.bio;
            if ((!artistDoc.genres || artistDoc.genres.length === 0) && data.genres && data.genres.length > 0) {
                artistDoc.genres = data.genres;
            }
            if (data.country) artistDoc.country = data.country;
            if (data.monthlyListeners) artistDoc.monthlyListeners = Number(data.monthlyListeners);
            if (!artistDoc.followers && data.followers) artistDoc.followers = Number(data.followers);
            if (data.verified !== undefined) artistDoc.verified = !!data.verified;
            
            if (data.instagram) artistDoc.instagram = data.instagram;
            if (data.youtube) artistDoc.youtube = data.youtube;
            if (!artistDoc.spotify && data.spotify) artistDoc.spotify = data.spotify;
            if (data.website) artistDoc.website = data.website;
            
            if (data.similarArtists) artistDoc.similarArtists = data.similarArtists;

            // Popular songs returned by Gemini
            if (data.popularSongs && data.popularSongs.length > 0) {
                artistDoc.moreSongs = data.popularSongs.map((title: string) => ({
                    title,

...
function mapToMacroCluster(genreStr) {
    ...
    for (const cluster of MACRO_CLUSTERS) {
        if (g.includes(cluster.replace('_', ' ')))
            return cluster;
    }
    if (g.includes("indie"))
        return "folk_indie";
    if (g.includes("house") || g.includes("techno") || g.includes("edm") || g.includes("dubstep") || g.includes("phonk"))
        return "electronic_dance";
    if (g.includes("rap") || g.includes("trap") || g.includes("drill"))
        return "hip_hop";
    if (g.includes("rnb") || g.includes("soul") || g.includes("funk"))
        return "r&b_soul";
    if (g.includes("acoustic") || g.includes("singer-songwriter") || g.includes("cottagecore"))
        return "folk_indie";
    if (g.includes("core") || g.includes("death") || g.includes("shred"))
        return "metal";
    if (g.includes("lo-fi") || g.includes("ambient") || g.includes("chill"))
        return "new_age_ambient";
    return "pop"; // fallback
}
...
function getQuadrantSimilarity(q1, q2) {
    if (q1 === q2)
        return 1.0;
    // Opposite check
    if ((q1 === "happy-energetic" && q2 === "sad-calm") || (q2 === "happy-energetic" && q1 === "sad-calm"))
        return 0.0;
    if ((q1 === "happy-calm" && q2 === "sad-energetic") || (q2 === "happy-calm" && q1 === "sad-energetic"))
        return 0.0;
    // Adjacent
    return 0.5;
}
...
class MusicRecommender {
    ...
    async init() {
        ...
        const historySongIds = Array.from(new Set(allHistory.map(h => h.songId.toString())));
        const missingSongIds = historySongIds.filter(id => !this.songCatalog.has(id));
        if (missingSongIds.length > 0) {
            ...
            for (const id of missingSongIds) {
                if (!foundSongIds.has(id)) {
                    this.invalidSongIds.add(id);
                }
            }
        }
    ...
    async recalculateUser(userId, precomputedHistory = null) {
        ...
        const missingSongIds = Array.from(new Set(songIds.filter(id => !this.songCatalog.has(id) && !this.invalidSongIds.has(id))));
        if (missingSongIds.length > 0) {
            const missingSongs = await Song.find({ _id: { $in: missingSongIds } }).select("title artist audio_details tempo energy valence acousticness danceability genre");
            const foundSongIds = new Set(missingSongs.map(s => s._id.toString()));
            for (const song of missingSongs) {
    ...
    calculateSimilarity(userA, userB) {
        ...
        let finalScore = ((genreOverlap * 0.30) +
            (audioSim * 0.25) +
            (moodAlignment * 0.20) +
            (artistOverlap * 0.15) +
            (contextSync * 0.10));
        ...
        let sharedGenres = [];
        for (const g of allGenres) {
            if (userA.genres[g] > 0.1 && userB.genres[g] > 0.1)
                sharedGenres.push(g);
        }
        let tensionPoints = [];
        if (moodAlignment === 0)
            tensionPoints.push("Opposing core moods (Happy/Sad vs Calm/Energetic)");
        if (audioSim < 0.5)
            tensionPoints.push("Differing sonic textures");
        if (contextSync === 0.5 && userA.preferredTime !== "Unknown")
            tensionPoints.push(`You listen in the ${userA.preferredTime}, they listen in the ${userB.preferredTime}`);
        let narrative = "";
        if (grade === "S" || grade === "A") {
            ...
            if (moodAlignment === 1)
                narrative += `Your emotional listening profiles align perfectly in the ${qA} quadrant.`;
        }
        else if (grade === "B") {
